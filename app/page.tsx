...
export default function FloorManagement() {
  ...
  const handleAddPoint = (e: React.MouseEvent) => {
    ...
    const x = (e.clientX - rect.left) / 1920
    const y = (e.clientY - rect.top) / 1080

    const newPoint = {
      x,
      y,
      ...newPointDetails,
      vlan: switchPorts.find((port) => port.port_number.toString() === newPointDetails.port)?.vlan,
      activo: switchPorts.find((port) => port.port_number.toString() === newPointDetails.port)?.status,
    }

    const updatedPoints = [...floors[currentFloorIndex].points, newPoint]

    setFloors((prev) =>
      prev.map((floor, index) => (index === currentFloorIndex ? { ...floor, points: updatedPoints } : floor)),
    )
  ...
  const handleMouseMove = (e: MouseEvent) => {
    if (!isCustomizing || !containerRef.current || !imageRef.current || isMapFixed) return

    const rect = containerRef.current.getBoundingClientRect()
    const newLeft = e.clientX - rect.left - dragOffset.x
    const newTop = e.clientY - rect.top - dragOffset.y

    setMapPosition({ x: newLeft / 1920, y: newTop / 1080 })
  }
  ...
  const handleSVGUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      setFloors((prev) =>
        prev.map((floor, index) => (index === currentFloorIndex ? { ...floor, svg: svgContent } : floor)),
      )
    }
    ...
    setFloors((prev) =>
      prev.map((floor, index) =>
        index === currentFloorIndex
          ? {
              ...floor,
              points: floor.points.map((point, i) => (i === draggedPoint ? { ...point, x, y } : point)),
            }
          : floor,
      ),
    )
  ...
  const deletePoint = (index: number) => {
    ...
    setFloors((prev) => prev.map((floor, i) => (i === currentFloorIndex ? { ...floor, points: updatedPoints } : floor)))
  ...
  return (
    <div className="h-[1080px] w-[1920px] relative">
      ...
      {error && (
        <div
          className="fixed top-4 right-4 z-50 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded shadow-lg"
          role="alert"
        >
      ...
      {floors.length > 0 && (
        <div className="h-[1080px] w-[1920px]">
          ...
          <div
            ref={containerRef}
            className="h-[1080px] w-[1920px] relative"
          ...
          >
            <div
              ...
              style={{
                ...
                transform: `translate(${mapPosition.x * 1920}px, ${mapPosition.y * 1080}px) scale(${zoomLevel})`,
          ...
          {isCustomizing && (
            <div className="fixed bottom-4 left-1/2 transform -translate-x-1/2 z-50 flex items-center gap-4 bg-white/80 backdrop-blur-sm rounded-lg p-2 shadow-lg">
              <Button onClick={() => setZoomLevel((prev) => Math.max(0.5, prev - 0.1))} variant="ghost">
                -
              </Button>
              <span>Zoom: {Math.round(zoomLevel * 100)}%</span>
              <Button onClick={() => setZoomLevel((prev) => Math.min(2, prev + 0.1))} variant="ghost">
                +
              </Button>
      ...
      {showManualPointForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            ...
            <ManualPointForm onSubmit={handleManualAddPoint} onCancel={() => setShowManualPointForm(false)} />
          </div>
        </div>
      )}
      <ColorLegend />
